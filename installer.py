...
log_file = os.path.join(os.path.dirname(__file__), 'roop.log')
quick_allowed = True
errors = 0
opts = {}
args = Dot({
...
def install_packages():
    ...
    install('onnxruntime-gpu==1.14.0', 'onnxruntime-gpu', ignore=True)
    if args.profile:
...
def add_args():
    ...
    group.add_argument('--profile', default = False, action='store_true', help = "Run with profiling enabled")
...
def run_setup():
    ...
    log.info('Starting roop-auto setup')
    #read_options()
    check_python()
    if args.reset:
        git_reset()
